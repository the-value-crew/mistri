...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    <form method="post" action="{{route('storeSubCategory')}}" class="form-horizontal" enctype="multipart/form-data">
                        @csrf
                        <div class="card-header" data-background-color="">

                            <div class="card-title">
                                <h4><b>Create Service Sub-Category</b></h4>
                                ...
                                <select class="form-control select2 select2-hidden-accessible" name="category_id"  data-placeholder="Select Category" style="width: 100%;" tabindex="-1" aria-hidden="true">
                                    ...
                                    @foreach ($categories as $category)
                                        ...
                                        @foreach($sub_categories as $sub_category)
                                            <option value="{{$sub_category->id}}" @if($sub_category->id == $id)selected @endif>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; -- &nbsp{{$sub_category->name}}</option>
                                            @php $sub_subs = \App\ServiceCategory::where('parent_id',$sub_category->id)->get(); @endphp
                                            @foreach($sub_subs as $sub_sub)
                                                <option value="{{$sub_sub->id}}"  @if($sub_sub->id == $id)selected @endif>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; -- &nbsp{{$sub_sub->name}}</option>
                                                @php $sub_level4s = \App\ServiceCategory::where('parent_id',$sub_sub->id)->get(); @endphp
                                                @foreach($sub_level4s as $sub_level4)
                                                    <option value="{{$sub_level4->id}}"  @if($sub_level4->id == $id)selected @endif>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; -- &nbsp{{$sub_level4->name}}</option>
                                                @endforeach
                                            @endforeach
                                        @endforeach
                                    @endforeach
